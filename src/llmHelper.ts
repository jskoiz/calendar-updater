...
export async function processUserInput(input: string): Promise<CalendarEvent[]> {
...
`;
    ...
    try {
        ...
        try {
            ...
            if (innerError instanceof Error) {
                logger.error('Failed to parse JSON array from LLM response:', innerError);
            } else {
                logger.error('Failed to parse JSON array from LLM response: Unknown error');
            }
            logger.debug(`LLM response content that failed to parse: ${response.response}`);
    ...
    return events.map(event => ({
        ...
        start: event.start ? ensureDateTime(event.start).toISO() || '' : '',
        end: event.end ? ensureDateTime(event.end).toISO() || '' : ''
    }));
}
...
${colorizeText("================================", "cyan")}`);
    ...
    console.log(colorizeText("\nThe following events have been generated based on your input:", "cyan"));
    console.log(colorizeText(eventList, "green"));
    console.log(colorizeText("\n================================", "cyan"));

    const confirmPrompt = await promptUser(colorizeText('\nDo you want to add these events to your calendar? (yes/no) ', "yellow"));
